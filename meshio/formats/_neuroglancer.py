...
Adapted from https://github.com/HumanBrainProject/neuroglancer-scripts/blob/1fcabb613a715ba17c65d52596dec3d687ca3318/src/neuroglancer_scripts/mesh.py
"""
import struct
...
def write_buffer(file, mesh):
    ...
    try:
        triangles = np.asarray(mesh.cells["triangle"])
    except KeyError:
        raise WriteError(
    ...
    if not np.can_cast(vertices.dtype, "<f"):
        warn("Vertex coordinates will be converted to float32")
...
def register():
    register_reader("neuroglancer", read, ".ngl")
    register_writer("neuroglancer", write, ".ngl")
